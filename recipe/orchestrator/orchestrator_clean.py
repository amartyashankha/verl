...
def compute_score(data_source: str, solution_str: str, ground_truth=None, extra_info=None, **kwargs) -> float:
    """Compute reward score for orchestrator agent responses using synchronous HTTP calls."""
    import httpx
    ...
    try:
        with httpx.Client(timeout=600) as client:
            logger.info(f"[COMPUTE_SCORE] Submitting to {modal_evaluation_url}/evaluate")
            ...
            logger.info(f"[COMPUTE_SCORE] Response status: {response.status_code}")
            
            if response.status_code == 200:
                reward_data = response.json()
                if not reward_data.get("success", False):
                    logger.error(f"[COMPUTE_SCORE] Evaluation failed: {reward_data.get('error')}")
                    return 0.0
                ...
                logger.info(f"[COMPUTE_SCORE] Results for {instance_id}:")
                logger.info(f"  Resolved: {resolved}")
                logger.info(f"  Fail-to-pass: {len(fail_to_pass)}")
                logger.info(f"  Pass-to-fail: {len(pass_to_fail)} (regressions)")
                logger.info(f"  Fail-to-fail: {len(fail_to_fail)}")
                
                if resolved:
                    score = 1.0
                elif total_originally_failing > 0:
                    score = len(fail_to_pass) / total_originally_failing
                    # Apply regression penalty
                    if len(pass_to_fail) > 0:
                        penalty = len(pass_to_fail) * 0.1  # 0.1 penalty per regression
                        score = max(0, score - penalty)
                else:
                    score = 0.0
                
                logger.info(f"[COMPUTE_SCORE] Final score for {instance_id}: {score:.3f}")
                return score
            else:
                logger.error(f"[COMPUTE_SCORE] HTTP {response.status_code}: {response.text[:500]}")
                return 0.0
    except Exception as e:
        logger.error(f"[COMPUTE_SCORE] Error evaluating {instance_id}: {e}")
        return 0.0
