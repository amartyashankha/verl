...
model_path=$DATA_ROOT/checkpoint/Qwen1.5-0.5B
...
experiment_name=qwen1.5-0.5b_orchestrator_modal
...
val_batch_size=1  # Process validation in batches to avoid overwhelming Modal API
...
if [[ "${DEBUG_MODE:-false}" == "true" ]]; then
    ...
    train_batch_size=1  # Must be <= number of training examples (4)
    ppo_mini_batch_size=1  # Must be <= train_batch_size
